...
def calculate_job_fit(work_style, work_env, interest_job):
    ...
    for job in job_category_map.keys():
        ...
        # --- [수정] 최종 점수가 100을 넘지 않도록 여기서 값을 제한합니다. ---
        final_score = min(100, score + 5)
        job_fit_scores[job] = final_score
...
with main_tabs[0]:
    ...
    with col1:
        ...
        st.progress(int(progress_value) / 100) # 이제 progress_value는 항상 100 이하입니다.

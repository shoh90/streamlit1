...
def load_data(_conn):
    ...
    try:
        ...
        if csv_files:
            ...
            rallit_df = rallit_df.drop_duplicates(subset=['url']).reset_index(drop=True)
...
job_category_map = {
    ...
    "마케팅": ["마케팅", "마케터", "Marketing", "광고", "콘텐츠"],
    "기획": ["기획", "PM", "PO", "서비스", "Product"],
    "프론트엔드": ["프론트엔드", "Frontend", "React", "Vue", "웹 개발"],
    "백엔드": ["백엔드", "Backend", "Java", "Python", "서버", "Node.js"],
    "AI/ML": ["AI", "ML", "머신러닝", "딥러닝", "인공지능"],
    "디자인": ["디자인", "디자이너", "Designer", "UI", "UX", "BX", "그래픽"],
    "영업": ["영업", "Sales", "세일즈", "비즈니스", "Business Development"],
    "고객지원": ["CS", "CX", "고객", "지원", "서비스 운영"],
    "인사": ["인사", "HR", "채용", "조직문화", "Recruiting"]
}
job_characteristics = {job: {"work_style": "분석적이고 논리적", "work_env": "독립적으로 일하기"} for job in job_category_map.keys()}

def calculate_job_fit(work_style, work_env, interest_job):
    """사용자 성향 기반 직무 적합도 계산"""
    job_fit_scores = {}
    for job, char_info in job_category_map.items():
        score = 0
        # 실제 서비스에서는 이 로직을 더 정교화해야 합니다.
        ...
        elif "계획" in work_style and any(k in job for k in ["기획", "인사"]): score += 50
        elif "사교" in work_style and any(k in job for k in ["영업", "마케팅", "고객지원"]): score += 50
        ...
        job_fit_scores[job] = score + 5 # 기본 점수
...
with main_tabs[0]:
    ...
    if rallit_df is not None:
        ...
        if all(col in rallit_df.columns for col in required_cols):
            ...
            # 'title' 컬럼에서 직무 키워드 검색
            job_mask = rallit_df["title"].str.contains(keyword_regex, case=False, na=False)
            
            # 경력 수준 필터링
            if career_level == "상관 없음":
                career_mask = pd.Series(True, index=rallit_df.index)
            elif career_level == "신입":
                career_mask = rallit_df["jobLevels"].str.contains("신입|경력 무관|신입~", case=False, na=False)
            else:
            ...
            st.error(f"Rallit 데이터 파일에 필수 컬럼('title', 'jobLevels')이 없습니다. CSV 파일의 컬럼명을 확인해주세요.")
    else:
...
with main_tabs[1]:
    ...
    with market_tabs[0]:
        ...
        fig_youth = px.bar(youth_df, x="성별", y=f"{selected_month}_실업률", color="성별", title=f"{selected_month} 성별 청년층 실업률", text_auto='.1f')
